...
import { AddpatientsdicuPage } from '../addpatientsdicu/addpatientsdicu';
import { SdicuPipe } from '../../pipes/search/sdicu';
import { EmergencyPage } from '../emergency/emergency';
@Component({
  selector: 'page-sdicu',
  templateUrl: 'sdicu.html',
})
export class SdicuPage {
  pipes: [SdicuPipe];
  Api_url = "";
  error_mes;
  array;
  ...
  ward_status;
  value;
  constructor(public navCtrl: NavController, public navParams: NavParams, public appCtrl: App, public popoverCtrl: PopoverController, public services: AllServiceProvider, public storage: Storage) {
  ...
  add() {
    this.appCtrl.getRootNavs()[0].push(AddpatientsdicuPage)
  }

  ionViewDidLoad() {
    console.log('ionViewDidLoad SdicuPage');
  }
  search() {
    this.value = 'search';
  }
  ...
  ngOnInit() {
    this.storage.get('patient_sdicu').then((val) => {
