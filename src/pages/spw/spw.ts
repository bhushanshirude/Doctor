...
import { AddpatientspwPage } from '../addpatientspw/addpatientspw';
import { SpwPipe } from '../../pipes/search/spw';
import { EmergencyPage } from '../emergency/emergency';
@Component({
  selector: 'page-spw',
  templateUrl: 'spw.html',
})
export class SpwPage {
  pipes: [SpwPipe];
  Api_url = "";
  error_mes;
  array;
  ...
  ward_status;
  value;
  constructor(public navCtrl: NavController, public navParams: NavParams, public popoverCtrl: PopoverController, public appCtrl: App, public storage: Storage, public services: AllServiceProvider) {
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad SpwPage');
  }

  add() {
    this.appCtrl.getRootNavs()[0].push(AddpatientspwPage);
  }
  search() {
    this.value = 'search';
  }
  ...
  ngOnInit() {
    this.storage.get('patient_spw').then((val) => {
      if (val != '' && val != undefined && val != null) {
        for (let i = 0; i <= val.length; i++) {
          this.array = val;
          console.log("SPw data", this.array);
        }
      } else {
