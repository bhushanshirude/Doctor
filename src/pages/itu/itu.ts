...
import { AddpatientituPage } from '../addpatientitu/addpatientitu';
import { ItuPipe } from '../../pipes/search/itu';
import { EmergencyPage } from '../emergency/emergency';

@Component({
  selector: 'page-itu',
  templateUrl: 'itu.html',
})
export class ItuPage {
  pipes: [ItuPipe];
  Api_url = "";
  error_mes;
  array;
  ...
  constructor(public navCtrl: NavController, public storage: Storage, public appCtrl: App, public popoverCtrl: PopoverController, public navParams: NavParams, public services: AllServiceProvider) {
    ...
    this.storage.get('patient_itu').then((val) => {
      console.log("patient_itu working", val)
    })
  ...
  ionViewDidLoad() {
    console.log('ionViewDidLoad ItuPage');
  }

  add() {
    this.appCtrl.getRootNavs()[0].push(AddpatientituPage)
  }
