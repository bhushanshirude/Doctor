...
import { AddpatienticcuPage } from '../addpatienticcu/addpatienticcu';
import { IccuPipe } from '../../pipes/search/iccu';
import { EmergencyPage } from '../emergency/emergency';
@Component({
  selector: 'page-iccu',
  templateUrl: 'iccu.html',
})
export class IccuPage {
  pipes: [IccuPipe];
  Api_url = "";
  error_mes;
  array;
  ...
  add() {
    this.appCtrl.getRootNavs()[0].push(AddpatienticcuPage)
  }

  ionViewDidLoad() {
    console.log('ionViewDidLoad IccuPage');
  }
  ...
  ngOnInit() {
    this.storage.get('patient_iccu').then((val) => {
